...
#include <Hephaestus/Controls/ControlManager.hpp>
...
void ControlManager::executeHolding() {
    for(Keybind & keybind : keybindings) {
        if(keybind.executesOnHold && heldKeys.count(keybind.keyCode) > 0 && keybind.action == GLFW_PRESS) { // We found the keybind is being held
            keybind.execute();
        }
    }
}
